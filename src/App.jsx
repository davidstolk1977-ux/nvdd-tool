      ...
      <div style={{ maxWidth: 880, margin: "0 auto", padding: "40px 24px" }}>
        ...
        {tab === "gasten" && (
          <div>
            ...
            <button onClick={slaGastOp} style={{ background: C.red, border: "none", color: "#fff", padding: "10px 24px", fontSize: 11, letterSpacing: 2, fontFamily: "monospace", textTransform: "uppercase", cursor: "pointer", marginBottom: 28 }}>+ Opslaan</button>
            {gasten.length === 0 && <div style={{ color: C.muted, fontSize: 14 }}>Nog geen gasten opgeslagen.</div>}
            {gasten.map(g => (
              <div key={g.id} style={{ background: "#fff", border: `1px solid ${C.border}`, padding: "14px 18px", display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <button onClick={() => { setName(g.naam); setBg(g.achtergrond); setTab("nieuw"); }} style={{ background: C.red, border: "none", color: "#fff", padding: "6px 14px", cursor: "pointer", fontSize: 11, fontFamily: "monospace" }}>Gebruik</button>
        ...
        {tab === "nieuw" && (
          <div>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              {(name || topics) && <button onClick={reset} style={{ background: "none", border: `1px solid ${C.border}`, color: C.muted, padding: "12px 20px", cursor: "pointer", fontSize: 11, fontFamily: "monospace", textTransform: "uppercase" }}>Nieuw</button>}
            </div>
            ...
            {prep && sel && (
              <div style={{ marginTop: 36 }}>
                ...
                <Blk label="PRES"><div style={{ fontSize: 16, lineHeight: 1.9, fontWeight: 500 }}>{prep.pres}</div></Blk>
                ...
                {prep.beeldvullers?.length > 0 && (
                  <div style={{ marginBottom: 22 }}>
                    {prep.beeldvullers.map((b, i) => <div key={i} style={{ fontSize: 15, fontWeight: 700, marginBottom: 6 }}>{b}</div>)}
                  </div>
                )}
